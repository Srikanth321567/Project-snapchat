...
const Friend = ({user}:{user:any}) => {
  ...
  if(amISender){
      ...
      icon = lastMessageType === 'text' ? (isMessageOpened ? <IoSend size={'16px'} className='text-[#00b4d8]'/> : <IoSendOutline size={'16px'} className='text-[#00b4d8]'/>)
  ...
  return (
    <Link href={`/chat/${user._id}`} className='flex gap-2 items-center justify-normal border-b-2 border-[#E3E6E8] my-2 p-3'>
